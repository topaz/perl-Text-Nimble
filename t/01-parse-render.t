...
subtest "parse+render html (list)" => sub {
  my $act_parsetree = Text::Nimble::parse($test_input);
  my ($act_html, $act_meta) = Text::Nimble::render(html => $act_parsetree);
  is($act_html, $exp_html);
...
subtest "render html (list)" => sub {
  my ($act_html, $act_meta) = Text::Nimble::render(html => $test_input);
  is($act_html, $exp_html);
  is_deeply($act_meta, $exp_parsetree->{meta});
};
...
<li>Why?</li>
<li>Why not?</li>
</ul>',format => 'html',type => 'raw'},{content => [{text => 'And we can end in just raw text, too.',type => 'text'}],type => 'paragraph'}]}; }
